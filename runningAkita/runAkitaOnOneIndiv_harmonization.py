...
for chrm,pos_list in chunks.items():
    ...
    for start_loc in pos_list:
        print("starting predictions on %s" % start_loc,flush=True)
        try: # some input start locations won't work because when + 1Mb they are past the end of the chromosome stop
            ...
            if (masked_coverage < 0.1) or (indiv_coverage < 0.1):
                lowCoverage=True
                continue
            else:
                lowCoverage=False
        ...
        f_coverage.write("%s\t%s\t%s\t%s\n" % (chrm,start_loc,indiv_coverage,masked_coverage))
        if not(lowCoverage):
