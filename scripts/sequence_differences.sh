...
ind1=$(awk -v row=$SGE_TASK_ID 'NR == row {print $1}' /wynton/group/capra/projects/modern_human_3Dgenome/data/reference/lists/baseline_sequence_pairs.txt)
ind2=$(awk -v row=$SGE_TASK_ID 'NR == row {print $2}' /wynton/group/capra/projects/modern_human_3Dgenome/data/reference/lists/baseline_sequence_pairs.txt)
